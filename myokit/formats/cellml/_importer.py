        ...
        for tag in model_tag.getElementsByTagName('connection'):
            ...
            for comp in (cname1, cname2):
                if comp not in components:
                    raise CellMLError(
                        'Connection found for unlisted component: <' + comp
                        + '>.')
        ...
        def dpp(lhs):
            ...
            if time is None:
                ...
                raise CellMLError(
                    ...
                    + str(time) + '> and <' + str(var) + '>.')

        # MathML expression parser
        def mathml(node, rfs):
            return parse_mathml_dom(
                node,
                var_table=rfs,
                logger=self.logger(),
            ...
            for m in math:
                ...
                while tag:
                    ...
                    if not isinstance(lhs, myokit.LhsExpression):
                        raise CellMLError(
                            'Error parsing equation: Expecting <ci> or <apply>'
...
def parse_mathml_dom(
    ...
    def parsex(node):
        ...
        def chain(kind, node, unary=None):
            ...
            n = len(ops)
            if n < 1:
                raise MathMLDomError('Operator needs at least one operand.')
            if n < 2:
                if unary:
                    return unary(ops[0])
                else:
                    raise MathMLDomError(
                        'Operator needs at least two operands')
            ex = kind(ops[0], ops[1])
            for i in range(2, n):
                ex = kind(ex, ops[i])
            return ex
    ...
    if kind == 'real':
        ...
        if base:
            try:
                ...
                raise MathMLDomError('Invalid BASE specified.')
            if base != 10:
                raise MathMLDomError(
                    'BASE conversion for reals is not supported')
        ...
        raise MathMLDomError('Unsupported <cn> type: ' + kind)
