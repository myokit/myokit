...
class EasyMLExporter(myokit.formats.Exporter):
    ...
    def model(self, path, model):
        ...
        with open(path, 'w') as f:
            # Write meta data
            f.write('/*' + eol)
            f.write('This file was generated by Myokit' + eol + eol)
            for key, val in model.meta.items():
                f.write(key + ': ' + val + eol)
            f.write(eol + '*/' + eol + eol)
            ...
            f.write('// Solve non-HH and non-Markov states with CVODE' + eol)
            f.write('group {' + eol)
            for v in model.states():
                if v == vm or v in markov_states or v in hh_states:
                    continue
                f.write('  ' + lhs(v) + eos)
            ...
            f.write('group {' + eol)
            for c in model.variables(const=True):
                f.write('  ' + lhs(c) + eos)
