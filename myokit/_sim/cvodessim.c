...
# Set of myokit.Name() objects for all parameters and all constants that depend on parameters
parameter_dependent_constants = set([myokit.Name(p) for p in parameters])

# Index of a parameter or state <variable> in the list of independent expressions
def s_index(var):
    try:
        return parameters.index(var)
    except ValueError:
...
# Get sensitivity output equations
...
for label, eqs in equations.items():
    for eq in eqs.equations(const=True):
        ...
        # Literals have already been set
        if eq.rhs.is_literal():
            continue

        # Parameters don't need setting here
        if eq.lhs.var() in parameter_set:
            continue

        # Variables that depend on parameters should not be set here
        if eq.rhs.references().intersection(parameter_dependent_constants):
            # Store this variable as a parameter-dependent constant
            parameter_dependent_constants.add(eq.lhs)
            continue
...
for label, eqs in equations.items():
    ...
    for eq in eqs.equations():
        ...
        # Parameters are set in the user data
        if var in parameter_set:
            continue

        # Constants are already set, except if they depend on a parameter
        if var.is_constant() and not eq.lhs in parameter_dependent_constants:
            continue
...
<?
for var in initials:
    print(tab + 'NV_Ith_S(sy[' + str(s_index(var)) + '], ' + str(var.indice()) + ') = 1.0;')
?>
    /*
    if (!dynamic_logging) {
        for(i=0; i<N_SENS; i++) {
...
<?
for var in initials:
    print(2*tab + 'NV_Ith_S(sy_log[' + str(s_index(var)) + '], ' + str(var.indice()) + ') = 1.0;')
?>    }
    */
...
for var in parameters:
    print(tab + 'udata->p[' + str(s_index(var)) + '] = ' + v(var, False) + ';')

# Initials are set to the current state value
for var in initials:
...
for var in model.variables(const=True, deep=True):
    if var.is_literal():
        ...
        print(tab + tab + v(var, False) + ' = value;')
