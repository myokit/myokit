...
sim_init(PyObject* self, PyObject* args)
{
    ...
    if(mcl_flag(clSetKernelArg(kernel_cell, 3, sizeof(arg_dt), &arg_dt))) return sim_clean();
    if(mcl_flag(clSetKernelArg(kernel_cell, 4, sizeof(nx_paced), &nx_paced))) return sim_clean();
    if(mcl_flag(clSetKernelArg(kernel_cell, 5, sizeof(ny_paced), &ny_paced))) return sim_clean();
    if(mcl_flag(clSetKernelArg(kernel_cell, 6, sizeof(arg_pace), &arg_pace))) return sim_clean();
    if(mcl_flag(clSetKernelArg(kernel_cell, 7, sizeof(mbuf_state), &mbuf_state))) return sim_clean();
...
sim_step(PyObject *self, PyObject *args)
{
    ...
    while(1) {
        ...
        if(mcl_flag(clSetKernelArg(kernel_cell, 6, sizeof(Real), &arg_pace))) return sim_clean();
