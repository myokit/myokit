...
Real *rv_y = NULL;  // rv = real vector
Real *rv_d = NULL;
int ds_y;           // ds = data size
int ds_d;
...
sim_clean()
{
    if(running) {
        ...
        clReleaseMemObject(mb_y);
        clReleaseMemObject(mb_y2);
        clReleaseMemObject(mb_f);
        clReleaseMemObject(mb_f2);
        clReleaseMemObject(mb_d);
        clReleaseMemObject(mb_ok);
        clReleaseKernel(kernel_cell);
        clReleaseKernel(kernel_diff);
        clReleaseKernel(kernel_feul);
        clReleaseKernel(kernel_heun);
        clReleaseKernel(kernel_test);
...
sim_init(PyObject* self, PyObject* args)
{
    ...
    for(i=0; i<n_total; i++) {
        ...
        rv_y[i] = (Real)PyFloat_AsDouble(flt);
    }
    
    // Create diffusion current vector and copy to device
    ds_d = nx*ny * sizeof(Real);
    rv_d = (Real*)malloc(ds_d);
    for(i=0; i<nx*ny; i++) rv_d[i] = 0.0;
    ...
    mb_y = clCreateBuffer(context, CL_MEM_READ_WRITE, ds_y, NULL, &flag);
    if(mcl_flag(flag)) return sim_clean();
    mb_y2 = clCreateBuffer(context, CL_MEM_READ_WRITE, ds_y, NULL, &flag);
    if(mcl_flag(flag)) return sim_clean();
    mb_f = clCreateBuffer(context, CL_MEM_READ_WRITE, ds_y, NULL, &flag);
    if(mcl_flag(flag)) return sim_clean();
    mb_f2 = clCreateBuffer(context, CL_MEM_READ_WRITE, ds_y, NULL, &flag);
    if(mcl_flag(flag)) return sim_clean();
    mb_d = clCreateBuffer(context, CL_MEM_READ_WRITE, ds_d, NULL, &flag);
    if(mcl_flag(flag)) return sim_clean();
    // Create OK buffer in host accessible memory
    mb_ok = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR, sizeof(ok), NULL, &flag);
    if(mcl_flag(flag)) return sim_clean();
    // Pin host_ok to ok buffer
    host_ok = clEnqueueMapBuffer(command_queue, mb_ok, CL_TRUE, CL_MAP_READ | CL_MAP_WRITE, 0, sizeof(ok), 0, NULL, NULL, &flag);
    if(mcl_flag(flag)) return sim_clean();
    ...
    flag = clEnqueueWriteBuffer(command_queue, mb_y, CL_TRUE, 0, ds_y, rv_y, 0, NULL, NULL);
    if(mcl_flag(flag)) return sim_clean();
    flag = clEnqueueWriteBuffer(command_queue, mb_d, CL_TRUE, 0, ds_d, rv_d, 0, NULL, NULL);
    if(mcl_flag(flag)) return sim_clean();
    #ifdef MYOKIT_DEBUG
    ...
    if(mcl_flag(flag)) return sim_clean();
    kernel_test = clCreateKernel(program, "test_error", &flag);
    if(mcl_flag(flag)) return sim_clean();
    #ifdef MYOKIT_DEBUG
    ...
    if(mcl_flag(clSetKernelArg(kernel_diff, 4, sizeof(mb_y), &mb_y))) return sim_clean();
    if(mcl_flag(clSetKernelArg(kernel_diff, 5, sizeof(mb_d), &mb_d))) return sim_clean();
    ...
    if(mcl_flag(clSetKernelArg(kernel_cell, 6, sizeof(mb_y), &mb_y))) return sim_clean();
    if(mcl_flag(clSetKernelArg(kernel_cell, 7, sizeof(mb_f), &mb_f))) return sim_clean();
    if(mcl_flag(clSetKernelArg(kernel_cell, 8, sizeof(mb_d), &mb_d))) return sim_clean();
    ...
    if(mcl_flag(clSetKernelArg(kernel_feul, 2, sizeof(mb_y), &mb_y))) return sim_clean();
    if(mcl_flag(clSetKernelArg(kernel_feul, 3, sizeof(mb_y2), &mb_y2))) return sim_clean();
    if(mcl_flag(clSetKernelArg(kernel_feul, 4, sizeof(mb_f), &mb_f))) return sim_clean();
    ...
    if(mcl_flag(clSetKernelArg(kernel_heun, 2, sizeof(mb_y), &mb_y))) return sim_clean();
    if(mcl_flag(clSetKernelArg(kernel_heun, 3, sizeof(mb_f), &mb_f))) return sim_clean();
    if(mcl_flag(clSetKernelArg(kernel_heun, 4, sizeof(mb_f2), &mb_f2))) return sim_clean();

    if(mcl_flag(clSetKernelArg(kernel_test, 0, sizeof(n_total), &n_total))) return sim_clean();
    if(mcl_flag(clSetKernelArg(kernel_test, 1, sizeof(arg_emax), &arg_emax))) return sim_clean();    
    if(mcl_flag(clSetKernelArg(kernel_test, 2, sizeof(mb_f), &mb_f))) return sim_clean();
    if(mcl_flag(clSetKernelArg(kernel_test, 3, sizeof(mb_f2), &mb_f2))) return sim_clean();
    if(mcl_flag(clSetKernelArg(kernel_test, 4, sizeof(mb_ok), &mb_ok))) return sim_clean();
...
for var in model.bindings_for('diffusion_current'):
    ...
    print(3*tab + 'if(log_add(log_dict, logs, vars, k_vars, log_var_name, &rv_d[i*nx+j])) {')
...
for var in model.states():
    if dims == 1:
        ...
        print(3*tab + 'sprintf(log_var_name, "%d.%d.' + var.qname() + '", j, i);' )
    print(3*tab + 'if(log_add(log_dict, logs, vars, k_vars, log_var_name, &rv_y[(i*nx+j)*n_state+' + str(var.indice()) + '])) {')
...
sim_step(PyObject *self, PyObject *args)
{
    ...
    while(1) {
        ...
        // 1. Evaluate diffusion current d(y)
        arg_time = (Real)engine_time;
        if(mcl_flag(clSetKernelArg(kernel_diff, 4, sizeof(mb_y), &mb_y))) return sim_clean();
        if(mcl_flag(clSetKernelArg(kernel_diff, 5, sizeof(mb_d), &mb_d))) return sim_clean();
        ...
        while(ok == 0) {
            ...
            // 3. Calculate y2 = y + dt*f(y)
            if(mcl_flag(clSetKernelArg(kernel_feul, 1, sizeof(arg_dt), &arg_dt))) return sim_clean();
            if(mcl_flag(clEnqueueNDRangeKernel(command_queue, kernel_feul, 1, NULL, &gws2, &lws2, 0, NULL, NULL))) return sim_clean();
            
            // 4. Evaluate diffusion current d(y2)
            if(mcl_flag(clSetKernelArg(kernel_diff, 4, sizeof(mb_y2), &mb_y2))) return sim_clean();
            if(mcl_flag(clSetKernelArg(kernel_diff, 5, sizeof(mb_d), &mb_d))) return sim_clean();
            if(mcl_flag(clEnqueueNDRangeKernel(command_queue, kernel_diff, 2, NULL, gws, lws, 0, NULL, NULL))) return sim_clean();
            
            // 5. Evaluate derivatives f(y2)
            arg_time = (Real)(engine_time + dt);
            if(mcl_flag(clSetKernelArg(kernel_cell, 2, sizeof(arg_time), &arg_time))) return sim_clean();
            if(mcl_flag(clSetKernelArg(kernel_cell, 6, sizeof(mb_y2), &mb_y2))) return sim_clean();
            if(mcl_flag(clSetKernelArg(kernel_cell, 7, sizeof(mb_f2), &mb_f2))) return sim_clean();
            if(mcl_flag(clEnqueueNDRangeKernel(command_queue, kernel_cell, 2, NULL, gws, lws, 0, NULL, NULL))) return sim_clean();
            
            // 6. Test if this step was small enough
            ok = 1;
            arg_emax = (Real)(2.0 * emax / dt);
            memcpy(host_ok, &ok, sizeof(ok));
            if(mcl_flag(clSetKernelArg(kernel_test, 1, sizeof(arg_emax), &arg_emax))) return sim_clean();    
            if(mcl_flag(clEnqueueNDRangeKernel(command_queue, kernel_test, 1, NULL, &gws2, &lws2, 0, NULL, NULL))) return sim_clean();
            if(mcl_flag(clFlush(command_queue))) return sim_clean();
            if(mcl_flag(clFinish(command_queue))) return sim_clean();
            memcpy(&ok, host_ok, sizeof(ok));
        ...
        if(engine_time >= tnext_log) {
            if(logging_diffusion) {
                if(mcl_flag(clEnqueueReadBuffer(command_queue, mb_d, CL_TRUE, 0, ds_d, rv_d, 0, NULL, NULL))) return sim_clean();
            }
            if(logging_states) {
                if(mcl_flag(clEnqueueReadBuffer(command_queue, mb_y, CL_TRUE, 0, ds_y, rv_y, 0, NULL, NULL))) return sim_clean();
            }
            for(i=0; i<n_vars; i++) {
                flt = PyFloat_FromDouble(*vars[i]);
                ret = PyObject_CallMethodObjArgs(logs[i], list_update_str, flt, NULL);
        ...
        //dt = default_dt;
        dt *= 1.02;
        x = tmax - engine_time; if(x < dt) dt = x;
        x = tnext_pace - engine_time; if(x < dt) dt = x;
    ...
    for(i=0; i<n_total; i++) {
        PyList_SetItem(state_out, i, PyFloat_FromDouble(rv_y[i]));
