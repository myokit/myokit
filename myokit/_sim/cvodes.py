...
import os
import myokit
import array    #TODO Used for sensitivities

# Location of C template
SOURCE_FILE = 'cvodes.c'


class Simulation2(myokit.CModule):
    """
    Runs single cell simulations using a CVODES backed. CVODES uses an implicit
    ...
    To calculate parameter sensitivities (``dy/dp``, where ``y`` is a state and
    ``p`` is a model parameter), a list of variables (or variable names) can be
    passed in as ``sensitivities``. If this is specified, the derivative of
    each state with resepect to the given parameters will be calculated and
    returned when simulations are run. All parameters must be literal values,
    i.e. they should not rely on any other variables.
    ...
    def __init__(self, model, protocol=None, apd_var=None, sensitivities=None):
        ...
        if sensitivities is None:
            ...
            self._sensitivities = []
            for var in sensitivities:
                if isinstance(var, myokit.Variable):
                    var = var.qname()
                var = self._model.get(var)
                if not var.is_literal():
                    raise ValueError(
                        'Sensitivities can only be calculated with respect to'
                        ' variables defined as literals.')
                self._sensitivities.append(var)
        ...
        Simulation2._index += 1
        module_name = 'myokit_sim_' + str(Simulation2._index)
    ...
    def run(
        ...
        If sensitivities are calculated, the returned value is
        ``(log, sensitivities)`` or ``(log, apds, sensitivities)`` if
        threshold crossings are calculated too.
    ...
    def _run(
        ...
        # Sensitivity
        #TODO: Come up with a good structure to use
        sensitivity_log = None
        if self._sensitivities is not None:
            #typecode = 'd' if precision == myokit.DOUBLE_PRECISION else 'f'
            sensitivity_log = array.array('d')
        ...
        with myokit.SubCapture() as capture:
            if duration > 0:
                ...
                try:
                    ...
                    if 'Function CVodes()' in e.message:
