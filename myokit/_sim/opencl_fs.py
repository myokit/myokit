...
SOURCE_FILE = 'opencl_fs.c'
KERNEL_FILE = 'opencl_fs.cl'
class SimulationOpenCLFS(myokit.CModule):
    """
    Can run 1d or 2d simulations based on a :class:`model <Model>` using
    OpenCL for parallelization. This simulation uses a forward Euler
    integration with different step sizes for fast and slow components.
    
    To use it, mark components in a model as "fast" by adding the meta
    property ``update: fast``. These components, and any components depending
    on them, will then be evaluated at every time step. The remaining
    components will only be updated once every ``r`` steps.
    ...
    ``dt``
        The time step to use for the slow parts of the model.
    ``r``
        A positive integer defining the ratio between the number of updates of
        the slow and the fast components. Fast components are updated at every
        time step, slow components only once every ``r`` steps.
    ``precision``
    ...
    has two repercussions:
      ...
      - Models used in a SimulationOpenCLFS need to have independent
    ...
    def __init__(self, model, protocol=None, ncells=256, npaced=5, g=9.5,
            dt=0.01, r=2, precision=myokit.SINGLE_PRECISION):
        super(SimulationOpenCLFS, self).__init__()
        # Which external variables are global?
        self._global = ['time', 'pace', 'time_step']
    ...
    def _create_backend(self):
        ...
        """
        # Get equations in solvable order
        equations = self._model.solvable_order()
        # Delete "*remaining*" group, guaranteed to be empty with independent
        # components
        del(equations['*remaining*'])
        # Get component order
        comp_order = equations.keys()
        comp_order = [self._model.get(c) for c in comp_order]
        # Get component input/output arguments
        # States and derivatives are stored in arrays, constants are macros
        comp_in, comp_out = self._model.map_component_io(
            omit_states = True,
            omit_constants = True,
            omit_derivatives = True)
        # Find components marked as fast
        fast = set()
        for k, c in enumerate(comp_order):
            if 'update' in c.meta and c.meta['update'] == 'fast':
                fast.add(c)
        # Find components dependent on diffusion current
        for v in self._model.bindings_for('diffusion_current'):
            fast.add(v.parent(myokit.Component))
        # Add dependent components
        comp_deps = self._model.map_component_dependencies()
        added = True
        while added:
            added = False
            for c, deps in comp_deps.iteritems():
                if c not in fast:
                    for d in deps:
                        if d in fast:
                            fast.add(c)
                            added = True
                            break
        # Put fast in solvable order
        fast = list(fast)
        fast.sort(key=lambda c: comp_order.index(c))
        # Get inputs to fast part
        # States and derivatives are already cached, bound variables will be
        # reset before evaluation
        fast_cache = set()
        for c in fast:
            for lhs in comp_in[c]:
                v = lhs.var()
                if not (v.is_state() or v.is_bound() or v in fast_cache
                        or v.parent(myokit.Component) in fast):
                    fast_cache.add(v)
        # Unique simulation id
        SimulationOpenCLFS._index += 1
        mname = 'myokit_sim_openclfs_' + str(SimulationOpenCLFS._index)
    ...
    def run(self, duration, log=None, log_interval=1.0, report_nan=True,
        ...
        number indicating the cell index. For example, when using::
            ...
            s = SimulationOpenCLFS(m, p, ncells=256)
                ...
                if n_states > 1:
                    ...
                    try:
                        derivs = self._model.eval_state_derivatives(states[1],
                            bound=bound[1], precision=self._precision)
