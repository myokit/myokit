...
# Myokit imports
import myokit
from myokit._sim import DataLog, prepare_log, dimco
# Location of C and OpenCL sources
SOURCE_FILE = 'opencl12d_adams2.c'
KERNEL_FILE = 'opencl12d_adams2.cl'
class SimulationOpenCLAdams2(myokit.CModule):
    """
    Can run 1d or 2d simulations based on a :class:`model <Model>` using
    OpenCL for parallelization and a two-step Adams-Bashforth update step. This
    method has a similar running time to Forward-Euler but has higher
    precision. The downside is a decreased stability region IE a larger step
    size.
    ...
    has two repercussions:
      ...
      - Models used in a SimulationOpenCLAdams2 need to have independent
    ...
    def __init__(self, model, protocol=None, ncells=256, npaced=5, g=9.5,
        ...
        super(SimulationOpenCLAdams2, self).__init__()
    ...
    def _create_backend(self):
        ...
        SimulationOpenCLAdams2._index += 1
        mname = 'myokit_sim_opencl_' + str(SimulationOpenCLAdams2._index)
    ...
    def find_nan(self, log, watch_var=None, safe_range=None):
        ...
        Returns a tuple ``var, value, time, states`` where ``var`` is the
        :class:`myokit.Variable` that first had a ``NaN``, while ``value`` is
        the value it had just before becoming ``NaN``. The time the ``NaN``
        occurred is given as ``time`` and the cell state just before, during
        and after encountering the ``NaN`` are given as ``states``.
        ...
        t = prepare_log(myokit.LOG_STATE+myokit.LOG_BOUND,
            self._model, dims=self._dims, global_vars=t)
        for key in t:
            if key not in log:
                raise Exception('Method requires a simulation log containing'
                    ' all states and bound variables. Missing variable <'
                    + key + '>.')
        del(t)
        # Error criterium
        if watch_var is None:
            ...
            try:
                ...
                raise ValueError('Variable <' + str(watch_var) + '> not'
                    ' found.')
            if not watch_var.is_state():
                raise ValueError('The watched variable must be a state.')
            try:
                lo, hi = safe_range
            except Exception:
                raise ValueError('A safe range must be specified for the'
                    ' watched variable as a tuple (lower, upper).')
            if lo >= hi:
                raise ValueError('The safe range must have a lower bound'
                    ' that is lower than the upper bound.')
        ...
        def relog(_log, _dt):
            ...
            if kfirst is None:
                raise Exception('Error condition not found in log.')
            if ifirst == 0:
                raise Exception('Unable to work with simulation logs where the'
                    ' error condition is met in the very first data point.')
        ...
        istate = icell*nstate       
        # Get state before, during and after error
        ...
        states = [state(ifirst - 1, icell),
                  state(ifirst    , icell),
                  state(ifirst + 1, icell),]
        ...
        # Return variable, value, states
        return var, value, time, states
    ...
    def run(self, duration, log=None, log_interval=1.0, progress=None,
        ...
        - An existing :class:`DataLog` object.
        ...
        number indicating the cell index. For example, when using::
            ...
            s = SimulationOpenCLAdams2(m, p, ncells=256)
