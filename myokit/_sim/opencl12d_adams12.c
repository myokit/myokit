...
sim_init(PyObject* self, PyObject* args)
{
    ...
    if(mcl_flag(flag)) return sim_clean();
    me = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(Real), NULL, &flag);
    if(mcl_flag(flag)) return sim_clean();        
    #ifdef MYOKIT_DEBUG
    ...
    //if(mcl_flag(flag)) return sim_clean();
    emax = clCreateKernel(program, "emax", &flag);
    if(mcl_flag(flag)) return sim_clean();
    #ifdef MYOKIT_DEBUG
    ...
    if(mcl_flag(clSetKernelArg(eval, 9, sizeof(arg_h1), &arg_h1))) return sim_clean();

    /*
    if(mcl_flag(clSetKernelArg(next, 0, sizeof(n_total), &n_total))) return sim_clean();
    if(mcl_flag(clSetKernelArg(next, 1, sizeof(arg_h1), &arg_h1))) return sim_clean();
    if(mcl_flag(clSetKernelArg(next, 2, sizeof(mf1), &mf1))) return sim_clean();
    if(mcl_flag(clSetKernelArg(next, 3, sizeof(my1), &my1))) return sim_clean();
    */
    
    if(mcl_flag(clSetKernelArg(emax, 0, sizeof(n_total), &n_total))) return sim_clean();
    if(mcl_flag(clSetKernelArg(emax, 1, sizeof(mf0), &mf0))) return sim_clean();
    if(mcl_flag(clSetKernelArg(emax, 2, sizeof(mf1), &mf1))) return sim_clean();
    if(mcl_flag(clSetKernelArg(emax, 3, sizeof(me), &me))) return sim_clean();
...
sim_step(PyObject *self, PyObject *args)
{
    ...
    while(1) {
        ...
        // Calculate mf1 = f(t1, y(t1)) and perform an Euler step
        arg_time = (Real)engine_time;
        arg_pace = (Real)engine_pace;
        arg_h1 = (Real)(h1);
        if(mcl_flag(clSetKernelArg(eval, 4, sizeof(arg_time), &arg_time))) return sim_clean();
        ...
        if(mcl_flag(clSetKernelArg(emax, 1, sizeof(mf0), &mf0))) return sim_clean();
        if(mcl_flag(clSetKernelArg(emax, 2, sizeof(mf1), &mf1))) return sim_clean();
        if(mcl_flag(clEnqueueNDRangeKernel(command_queue, emax, 1, NULL, &gws2, &lws2, 0, NULL, NULL))) return sim_clean();
        if(mcl_flag(clEnqueueReadBuffer(command_queue, me, CL_TRUE, 0, sizeof(Real), &e, 0, NULL, NULL))) return sim_clean();
        
        // Set next time step
        if (e > 1e-5) {
            double eps = .01;
            double hnew = sqrt(2.0 * h1 * eps / e);
            h0 = h1;
            h1 = hnew;
            if (h1 < default_h) h1 = default_h;
        }
        ...
        if(engine_time >= tnext_log) {
            if(logging_diffusion) {
                if(mcl_flag(clEnqueueReadBuffer(command_queue, md1, CL_TRUE, 0, ds_d, rv_md1, 0, NULL, NULL))) return sim_clean();
            }
            if(logging_states) {
                if(mcl_flag(clEnqueueReadBuffer(command_queue, my1, CL_TRUE, 0, ds_y, rv_my1, 0, NULL, NULL))) return sim_clean();
            }
            for(i=0; i<n_vars; i++) {
                flt = PyFloat_FromDouble(*vars[i]);
                ret = PyObject_CallMethodObjArgs(logs[i], list_update_str, flt, NULL);
