...
for var in model.states():
    print('#define _d' + var.uname() + ' f[off + ' + str(var.indice()) + ']')
...
for comp, ilist in comp_in.iteritems():
    ...
    # Function header
    args = ['const int cid', 'const int off', '__global Real *y', '__global Real *f']
 ...
 *  y        : The state vector
 *  f        : The state derivatives vector
 */
__kernel void eval(
    const int nx,
    const int ny,
...
print(tab + '// Perform Euler step')
for var in model.states():
    print(tab + v(var) + ' += h1 * ' + v(var.lhs())) + ';'
?>
 ...
 *  h1 : The time step from t1 to t2
 *  f1 : The derivatives at t1
 *  y1 : The state vector at t1, will be updated to t2
 */
__kernel void next(
    const int nx,
    const Real h1,
    __global const Real *f1,
    __global Real *y
    )
{
...
/*
 *
 * Unparallelized max error finding
 *
 * Arguments
 *  nx : The numer of elements in the vector
 *  f0 : The derivatives at t0
 *  f1 : The derivatives at t1
 *  e  : The maximum absolute error found
 */
__kernel void emax( 
    const int nx,
    __global const Real *f0,
    __global const Real *f1,
    __global Real *e)
{
    int i;
    __global const Real *a = f0;
    __global const Real *b = f1;
    a += i_vm;
    b += i_vm;
    Real d = 0;
    for(i=i_vm; i<nx; i+=n_state) {
        d = fmax(d, fabs(*a - *b));
        a+=n_state;
        b+=n_state;
    }
