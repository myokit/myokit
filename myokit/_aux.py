...
class StreamCapture(object):
    """
    Redirects Python's standard output and/or error streams, temporarily
    catching (and suppressing) output.
    ...
    output. This can still be useful when the goal is to suppress, rather than
    capture, output.

    A :class:`StreamCapture` will only capture output generated by Python code,
    not by C or C++ extensions or subprocesses. For that, use
...
class ProcessOutputCapture(object):
    """
    Context-manager that redirects the current process's standard output and/or
    error streams using file-descriptor duplication, thereby temporarily
    catching (and suppressing) output.
    ...
    def __init__(self):
        ...
        # Are we already capturing? If someone enters the same context twice
