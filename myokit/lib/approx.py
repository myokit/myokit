...
class PiecewisePolynomial(FittedFunction):
    ...
    def __init__(self, p, k=None):
        ...
        if len(k) + 1 != n:
            raise ValueError(
                'The number of polynomial pieces given must be one more than'
                ' the number of splitting points.')
    ...
    *Extends:* :class:`PiecewisePolynomial`
    ...
    def __init__(self, p, k=None):
        if k is None:
            ...
            if len(p) + 1 != len(k):
                raise ValueError(
                    'The number of polynomial pieces given must be one less'
                    ' than the number of knots.')
...
def fit_lagrange_polynomial(f, i, n=None, x=None):
    ...
    if a >= b:
        raise ValueError(
            'The first point in the interval must be smaller than the second.')
    # Check function
    try:
        f(a)
    except TypeError:
        raise ValueError(
            'The given function must have a single positional argument.')
    ...
    if x is None:
        ...
        try:
            ...
            raise ValueError(
                'The given value for x must be a sequence type containing'
                ' n + 1 points on the interval i.')
        if n is None:
            n = m - 1
        elif n != m - 1:
            raise ValueError(
                'The given value for x must contain n+1 points on the interval'
                ' i.')
...
def fit_remez_polynomial(f, i, n, stop_tol=1e-6, max_iter=100):
    ...
    try:
        ...
        raise ValueError(
            'The second argument to remez() must be an interval specified as a'
            ' sequence type (a,b).')
    a, b = i
    if a >= b:
        raise ValueError(
    ...
    try:
        ...
        raise ValueError(
            'The first argument to remez() must be a callable function with a'
            ' single positional argument.')

    # Constrained minimization
    def fmin(e, x, a, b):
        return fmin_tnc(
            e, (x, ), bounds=((a, b), ), messages=0, approx_grad=True)[0]
...
def fit_cubic_spline(
        f, i, reltol=1e-3, abstol=None, min_pieces=2, max_pieces=50,
...
#
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
def suitable_expressions(
        model, var, blacklist=None, require_exp=False,
...
class PolynomialFitter(Fitter):
    ...
    def _fit(self, model, lhs, rng, report):
        ...
        if report:
            ...
            div = page.append('div')
            div.append('p').text(
                'Attempting to approximate expressions with a single'
                ' polynomial of at most degree '
                + str(self._max_order) + '.')
            div.append('p').text(
                'Accepting approximations if the (estimated) maximum error'
                ' divided by the (estimated) range is less than '
                + str(self._reltol))
        ...
        for lhs, rhs in suitable:
            ...
            if report:
                ...
                if fit:
                    div.append('h3').text(
                        'Found approximation of order ' + str(n))
                    div.append('p').math(fit)
                    div.append('p').text(
                        '(Maximum deviation / range) is approximately '
                        + str(deviation))
                else:
                    div.append('h3').text('No approximation found')
                    div.append('p').text(
                        'Approximation of order ' + str(n) + ' failed with an'
                        ' estimated maximum relative deviation of '
                        + str(deviation) + '.')
                if g:
                    # Add plot to report
                    box = ax.get_position()
...
class CubicSplineFitter(Fitter):
    ...
    def __init__(
            self, abstol=None, reltol=0.001, min_pieces=1, max_pieces=200,
            blacklist=None):
    ...
    def _fit(self, model, lhs, rng, report):
        ...
        if report:
            ...
            div = page.append('div')
            div.append('p').text(
                'Attempting to approximate equations using a'
                ' piecewise cubic spline containing at most '
                + str(self._max_pieces) + ' pieces.')
            div.append('p').text(
                'Accepting approximations if the (estimated)'
        ...
        suitable = suitable_expressions(
            model, lhs, blacklist=self._blacklist)
        count = 0
        for v, rhs in suitable:
            ...
            if report:
                ...
                if fit:
                    div.append('h3').text(
                        'Accepted approximation using ' + str(n) + ' pieces.')
                else:
