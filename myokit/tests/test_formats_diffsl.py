...
""")
    ...
    def test_inputs_and_outputs(self):
        # Tests custom inputs and outputs parameters
        ...
        with TemporaryDirectory() as d:
            ...
            path = d.path("test_inputs.diffsl")
            inputs = [model.get("membrane.C")]
            e.model(path, model, inputs=inputs)
            with open(path, "r") as f:
                content = f.read()
            self.assertIn("in_i {\n  membraneC = ", content)
            # Verify input is not in constants section
            self.assertNotIn("membraneC {", content)

            # Test with custom outputs
            path = d.path("test_outputs.diffsl")
            outputs = [model.get("membrane.V"), model.get("ina.m")]
            e.model(path, model, outputs=outputs)
            with open(path, "r") as f:
                content = f.read()
            # Check that outputs are in specified order (not alphabetical)
            lines = content.split("\n")
            out_section = []
            in_out = False
            for line in lines:
                ...
                if in_out:
                    if "}" in line:
                        break
                    out_section.append(line.strip())
            self.assertEqual(len(out_section), 2)
            self.assertIn("membraneV", out_section[0])
            self.assertIn("inaM", out_section[1])

            # Test with both custom inputs and outputs
            path = d.path("test_both.diffsl")
            e.model(path, model, inputs=inputs, outputs=outputs)
            with open(path, "r") as f:
                content = f.read()
            self.assertIn("in_i {\n  membraneC = ", content)
            # Verify outputs are present in out_i block in correct order
            match = re.search(r"out_i\s*\{([^}]*)\}", content, re.DOTALL)
            self.assertIsNotNone(match)
            out_content = match.group(1)
            ...
            with self.assertRaisesRegex(
                myokit.ExportError, "engine.pace must be a constant"
            ):
    ...
    def test_inputs_outputs_validation(self):
        ...
        with TemporaryDirectory() as d:
            ...
            with self.assertRaisesRegex(
                myokit.ExportError, "Input variable .* must be a constant"
            ):
                e.model(
                    path,
                    model,
                    inputs=[
                        model.get("membrane.C"),  # Valid
                        model.get("membrane.V"),  # Invalid (state)
                    ],
                )

            # Test 5: Multiple invalid outputs
            with self.assertRaisesRegex(
                myokit.ExportError, "Output variable .* must be time-varying"
            ):
                e.model(
                    path,
                    model,
                    outputs=[
                        model.get("membrane.V"),  # Valid (state)
                        model.get("ina.gNa"),  # Invalid (constant)
                    ],
                )

            # Test 6: Input from different model
...
class DiffSLExpressionWriterTest(myokit.tests.ExpressionWriterTestCase):
    ...
    def test_if_expressions(self):
        ...
        self.eq(
            If(Equal(a, b), c, d),
            "piecewise(heaviside(a - b) * heaviside(b - a) - 0.5, c, d)",
        )

        self.eq(
            If(Equal(a, b), c, Number(0)),
            "piecewise(heaviside(a - b) * heaviside(b - a) - 0.5, c, 0.0)",
        )

        self.eq(
            If(Equal(a, b), Number(0), d),
            "piecewise(heaviside(a - b) * heaviside(b - a) - 0.5, 0.0, d)",
        )

        self.eq(
            If(Equal(a, b), c, Number(1)),
            "piecewise(heaviside(a - b) * heaviside(b - a) - 0.5, c, 1.0)",
        )

        self.eq(
            If(Equal(a, b), Number(1), d),
            "piecewise(heaviside(a - b) * heaviside(b - a) - 0.5, 1.0, d)",
        )

        self.eq(
            If(NotEqual(a, b), c, d),
            "piecewise((1 - heaviside(a - b) * heaviside(b - a)) - 0.5, c, d)",
        )

        self.eq(
            If(More(a, b), c, d),
            "piecewise((1 - heaviside(b - a)) - 0.5, c, d)",
        )

        self.eq(
            If(MoreEqual(a, b), c, d),
            "piecewise(heaviside(a - b) - 0.5, c, d)",
        )

        self.eq(
            If(Less(a, b), c, d),
            "piecewise((1 - heaviside(a - b)) - 0.5, c, d)",
        )

        self.eq(
            If(LessEqual(a, b), c, d),
            "piecewise(heaviside(b - a) - 0.5, c, d)",
        )
    ...
    def test_piecewise_expressions(self):
        ...
        self.eq(
            ...
            "piecewise(heaviside(a - b) * heaviside(b - a) - 0.5, c, "
            "heaviside(a - d) * heaviside(d - a) - 0.5, 3.0, 4.0)",
        )

        self.eq(
            Piecewise(Less(a, b), Number(0), Less(c, d), Number(0), Number(5)),
            "piecewise((1 - heaviside(a - b)) - 0.5, 0.0, "
            "(1 - heaviside(c - d)) - 0.5, 0.0, 5.0)",
        )
