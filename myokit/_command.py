...
#
# This module should only ever be imported by __main__.py
#
...
def update():
    ...
    if local_version_int < supported_int:
        ...
        text = ('The current updater version (' + local_version + ') is unable'
                ...
                ' perform a manual update by downloading the new source files'
                ' from')
    ...
    try:  # Should delete path_update when done
        ...
        if (os.path.isfile(path_settings)):
            ...
            shutil.move(
                path_settings, os.path.join(path_current, 'old_settings.txt'))
    finally:
        # Delete update path
...
def add_update_parser(subparsers):
    """
    Adds a subcommand parser for the `update` command.
    """
