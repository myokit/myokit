/*
Luo-Rudy 1991
Generated on 2020-07-14 10:42:56
...
#define MYOKIT_SUNDIALS_VERSION 30200
...
/* Set initial values */
static void
default_initial_values(N_Vector y, void *user_data)
{
    UserData fdata;
    fdata = (UserData) user_data;
...
/* Show output */
static void PrintOutput(realtype t, realtype y, N_Vector* yS)
{
    /* Get sensitivities w.r.t. parameter 0 and 1 */
    realtype *s1, *s2, *s3;
    s1 = N_VGetArrayPointer(yS[0]);
    s2 = N_VGetArrayPointer(yS[1]);
    s3 = N_VGetArrayPointer(yS[2]);

    #if defined(SUNDIALS_EXTENDED_PRECISION)
        printf("%4.6Le     %14.6Le     %14.6Le     %14.6Le     %14.6Le\n", t, y, s1[0], s2[0], s3[0]);
...
int main()
{
    ...
    N_Vector y = NULL;
    N_Vector dy = NULL;
    N_Vector *yS = NULL;        /* Array of vectors, one vector per parameter */
    UserData fdata = NULL;
    void *cvode_mem = NULL;
    realtype pbar[N_SENS];      /* Scale of each parameter */
    ...
    if (check_flag((void*)yS, "N_VNew_Serial", 0)) goto error;
    ...
    /* Create parameter vector for sensitivities in user data */
    fdata = (UserData) malloc(sizeof *fdata);
    if (fdata == 0) goto error;
    fdata->p[0] = 16.0;     /* AC_gNa */
    fdata->p[1] = 0.09;     /* AC_gCa */
    fdata->p[2] = 0.0002;   /* Cai[t0] */

    /* Create parameter scale vector */
    /* TODO: Let user provide this? */
    for(i=0; i<N_SENS; i++) {
        pbar[i] = (fdata->p[i] == 0.0 ? 1.0 : fdata->p[i]);
    }
    ...
    /* Set initial values */
    default_initial_values(y, fdata);
    ...
    /* Note: NULL here is the place to insert a user function to calculate the
       RHS of the sensitivity ODE */
    flag = CVodeSensInit(cvode_mem, N_SENS, CV_STAGGERED, NULL, yS);
    if(check_flag(&flag, "CVodeSensInit", 1)) goto error;

    /* Attach user data */
    flag = CVodeSetUserData(cvode_mem, fdata);
    if (check_flag(&flag, "CVodeSetUserData", 1)) return(1);

    /* Set parameter scales used in tolerances */
    flag = CVodeSetSensParams(cvode_mem, fdata->p, pbar, NULL);
    if (check_flag(&flag, "CVodeSetSensParams", 1)) goto error;

    /* Set sensitivity tolerances calculating method (using pbar) */
    flag = CVodeSensEEtolerances(cvode_mem);
    if (check_flag(&flag, "CVodeSensEEtolerances", 1)) goto error;
    ...
    while(1) {
        ...
        if (flag == CV_SUCCESS) {
            ...
            if (t > tNext) {
                ...
                flag = CVodeGetSensDky(cvode_mem, tNext, 0, yS);
                if (check_flag(&flag, "CVodeGetSensDky", 1)) goto error;
                t = tNext;
                flag = CVodeReInit(cvode_mem, t, y);
                if (check_flag(&flag, "CVodeReInit", 1)) goto error;
                flag = CVodeSensReInit(cvode_mem, CV_STAGGERED, yS);
                if (check_flag(&flag, "CVodeSensReInit", 1)) goto error;

                /* Recalculate logging values at this point in time */
                rhs(t, y, dy, fdata);
            } else {
                flag = CVodeGetSens(cvode_mem, &t, yS);
                if (check_flag(&flag, "CVodeGetSens", 1)) goto error;
            }
            /* Event over? */
            if (fire != 0 && t >= tDown) {
                pace = 0;
...
error:
    ...
    N_VDestroyVectorArray(yS, N_SENS);
    CVodeFree(&cvode_mem);
    free(fdata);
